...
from vllm_omni.diffusion.diffusion_kv import paged_attention_runtime as runtime_module
...
def _make_runtime(
    ...
    *,
    ...
    num_kv_heads: int = 2,
    capacity: int = 16,
) -> tuple[DiffusionPagedAttentionRuntime, _FakeBlockTables, _FakeLayer, list[tuple]]:
    events: list[tuple] = []
    ...
    binding_map = {
        ("req-0", 0): DiffusionKVSequenceBinding(2, capacity),
        ("req-1", 1): DiffusionKVSequenceBinding(3, capacity),
    }

    monkeypatch.setattr(runtime_module, "set_current_vllm_config", lambda _config: nullcontext())
    ...
    monkeypatch.setattr(runtime_module.current_omni_platform, "build_diffusion_kv_attn_metadata", build_metadata)
    monkeypatch.setattr(
        runtime_module,
    ...
    runtime = DiffusionPagedAttentionRuntime(
        ...
        resolve_sequence=lambda request_id, sequence_id: binding_map[(request_id, sequence_id)],
    )
    return runtime, block_tables, layer, events
...
def test_prepare_batch_reuses_native_block_table_and_metadata_builders(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, block_tables, _, events = _make_runtime(monkeypatch)
    sequences = (
...
def test_omni_paged_backend_consumes_context_and_restores_diffusion_shape(
    ...
    runtime, _, layer, events = _make_runtime(monkeypatch)
    ...
    with runtime.activate(batch):
        context = runtime.prepare_layer_context("layer-0", query, key, value)
...
def test_paged_adapter_accepts_native_gqa_shapes(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, _, layer, _ = _make_runtime(monkeypatch, num_heads=32, num_kv_heads=8)
    batch = runtime.prepare_batch(
        [DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=3, seq_len=3)]
    )
...
def test_omni_paged_backend_defers_backend_owned_cache_update(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, _, layer, _ = _make_runtime(monkeypatch)
    ...
    with runtime.activate(batch):
        context = runtime.prepare_layer_context("layer-0", qkv, qkv, qkv)
        output = _run_omni_paged_backend(context)
...
def test_omni_piecewise_backend_defers_backend_owned_cache_update(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    layer.attn_backend.forward_includes_kv_cache_update = True
    batch = runtime.prepare_batch(
        [DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=4, seq_len=4)]
    )
    qkv = torch.randn(1, 4, 2, 4)
    metadata = SimpleNamespace(full_attn_spans=[[(1, 3)]], extra={})

    with runtime.activate(batch):
        context = runtime.prepare_layer_context(
            "layer-0",
            qkv,
            qkv,
            qkv,
...
def test_omni_paged_backend_runs_unequal_hunyuan_prefixes_without_padding(
    ...
    runtime, _, layer, events = _make_runtime(monkeypatch, capacity=64)
    batch = runtime.prepare_batch(
        [
            DiffusionPagedAttentionSequence(
                request_id="req-0",
...
def test_forward_rejects_unaligned_heterogeneous_piecewise_spans_before_cache_write(
    ...
    batch = runtime.prepare_batch(
        [
            DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=4, seq_len=4),
            DiffusionPagedAttentionSequence(request_id="req-1", sequence_id=1, query_len=4, seq_len=4),
        ]
    )
    ...
    with runtime.activate(batch), pytest.raises(ValueError, match="produce aligned segments"):
        runtime.prepare_layer_context(
            "layer-0",
            qkv,
            qkv,
...
def test_forward_rejects_invalid_piecewise_spans_before_cache_write(
    ...
    with runtime.activate(batch), pytest.raises(ValueError, match="sorted, non-overlapping, and within the sequence"):
        runtime.prepare_layer_context(
            "layer-0",
            qkv,
            qkv,
...
def test_forward_rejects_non_4d_mask_with_piecewise_spans_before_cache_write(
    ...
    with runtime.activate(batch), pytest.raises(NotImplementedError, match="requires a 4D tensor"):
        runtime.prepare_layer_context(
            "layer-0",
            qkv,
            qkv,
...
def test_forward_rejects_piecewise_query_ranges_before_cache_write(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with runtime.activate(batch), pytest.raises(NotImplementedError, match="query_ranges"):
        runtime.prepare_layer_context(
            "layer-0",
            qkv,
            qkv,
...
def test_forward_rejects_piecewise_non_suffix_query_before_cache_write(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with runtime.activate(batch), pytest.raises(ValueError, match="requires each query/write span to end"):
        runtime.prepare_layer_context(
            "layer-0",
            qkv,
            qkv,
...
def test_forward_rejects_untranslated_omni_metadata_before_cache_write(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, _, layer, _ = _make_runtime(monkeypatch)
    batch = runtime.prepare_batch(
        [DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=1, seq_len=1)]
    )
    qkv = torch.randn(1, 2, 4)
    metadata = SimpleNamespace(attn_mask=torch.ones(1, 1), extra={})

    with runtime.activate(batch), pytest.raises(NotImplementedError, match="cannot translate"):
        runtime.prepare_layer_context("layer-0", qkv, qkv, qkv, omni_attn_metadata=metadata)

    assert layer.updates == []


def test_forward_accepts_flattened_hidden_dimension(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, _, _, _ = _make_runtime(monkeypatch)
    batch = runtime.prepare_batch(
        [DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=3, seq_len=3)]
    )
    ...
    with runtime.activate(batch):
        context = runtime.prepare_layer_context("layer-0", query, query, query)
...
def test_forward_accepts_one_flattened_token_for_single_head(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, _, _, _ = _make_runtime(monkeypatch, num_heads=1, num_kv_heads=1)
    batch = runtime.prepare_batch(
        [DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=1, seq_len=1)]
    )
    qkv = torch.randn(1, 4)
...
def test_forward_requires_active_prepared_batch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="runtime.activate"):
        runtime.prepare_layer_context("layer-0", qkv, qkv, qkv)
...
def test_new_preparation_invalidates_native_buffer_views_in_older_batch(
    ...
    runtime, _, _, _ = _make_runtime(monkeypatch)
    old_batch = runtime.prepare_batch(
        [DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=1, seq_len=1)]
    )
    runtime.prepare_batch([DiffusionPagedAttentionSequence(request_id="req-1", sequence_id=1, query_len=1, seq_len=1)])

    with pytest.raises(ValueError, match="stale"):
        with runtime.activate(old_batch):
            pass
...
def test_block_table_change_invalidates_prepared_native_buffer_views(
    ...
    runtime.invalidate_prepared_batches()
...
def test_block_table_change_is_rejected_during_active_forward(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with runtime.activate(batch), pytest.raises(RuntimeError, match="during an active forward"):
        runtime.invalidate_prepared_batches()
...
def test_prepare_batch_rejects_active_native_buffers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with runtime.activate(batch), pytest.raises(RuntimeError, match="during an active forward"):
        runtime.prepare_batch(
            [DiffusionPagedAttentionSequence(request_id="req-1", sequence_id=1, query_len=1, seq_len=1)]
        )
...
def test_forward_rejects_tensor_count_different_from_prepared_batch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with runtime.activate(batch), pytest.raises(ValueError, match="token count"):
        runtime.prepare_layer_context("layer-0", qkv, qkv, qkv)
...
def test_forward_rejects_dtype_different_from_model_activation_dtype(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with runtime.activate(batch), pytest.raises(ValueError, match="model activation dtype"):
        runtime.prepare_layer_context("layer-0", qkv, qkv, qkv)
...
def test_forward_rejects_invalid_key_token_count(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with runtime.activate(batch), pytest.raises(ValueError, match="key token count"):
        runtime.prepare_layer_context("layer-0", query, key, query)


def test_forward_rejects_batched_layout_that_does_not_match_sequences(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, _, _, _ = _make_runtime(monkeypatch)
    batch = runtime.prepare_batch(
        [
            DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=1, seq_len=1),
            DiffusionPagedAttentionSequence(request_id="req-1", sequence_id=1, query_len=3, seq_len=3),
        ]
    )
    qkv = torch.randn(2, 2, 2, 4)

    with runtime.activate(batch), pytest.raises(ValueError, match="must match prepared sequences"):
        runtime.prepare_layer_context("layer-0", qkv, qkv, qkv)
...
def test_forward_rejects_padded_full_kv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with runtime.activate(batch), pytest.raises(ValueError, match="batched key layout must match prepared sequences"):
        runtime.prepare_layer_context("layer-0", query, full_kv, full_kv)


def test_causal_batch_rejects_non_suffix_write(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, _, _, _ = _make_runtime(monkeypatch, non_causal=False)
    sequence = DiffusionPagedAttentionSequence(
        request_id="req-0",
        sequence_id=0,
        query_len=2,
        seq_len=8,
    ...
    batch = runtime.prepare_batch([sequence])
    qkv = torch.randn(2, 2, 4)
...
def test_prepare_batch_rejects_native_decode_after_prefill_order(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime, block_tables, _, _ = _make_runtime(
        monkeypatch,
        non_causal=False,
        reorder_batch_threshold=1,
    )
    sequences = [
        DiffusionPagedAttentionSequence(request_id="req-0", sequence_id=0, query_len=2, seq_len=2),
        DiffusionPagedAttentionSequence(request_id="req-1", sequence_id=1, query_len=1, seq_len=1),
    ]

    with pytest.raises(ValueError, match="decode/short-query sequences before"):
        runtime.prepare_batch(sequences)

    assert block_tables.gather_calls == []
    assert block_tables.slot_calls == []


def test_prepare_batch_rejects_seq_len_beyond_installed_binding_capacity(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, block_tables, _, _ = _make_runtime(monkeypatch)
    ...
    with pytest.raises(ValueError, match="binding contains only 1 blocks"):
        runtime.prepare_batch([sequence])
...
def test_prepare_batch_rejects_seq_len_beyond_installed_logical_length(
    ...
    runtime, block_tables, _, _ = _make_runtime(monkeypatch)
    runtime.resolve_sequence = lambda *_args: DiffusionKVSequenceBinding(2, 4)
    sequence = DiffusionPagedAttentionSequence(
        request_id="req-0",
        sequence_id=0,
        query_len=5,
...
def test_sequence_contract_rejects_invalid_identity_or_span(kwargs: dict, message: str) -> None:
    values = {
        "request_id": "req-0",
        "query_len": 1,
        "seq_len": 1,
    ...
    with pytest.raises(ValueError, match=message):
        DiffusionPagedAttentionSequence(**values)
...
def test_layer_adapter_accepts_platform_native_backend_and_uses_rank_local_heads(
    ...
    monkeypatch.setattr(runtime_module, "get_attn_backend", select_backend)
    monkeypatch.setattr(runtime_module, "set_current_vllm_config", lambda _config: nullcontext())
    ...
    native_layer = runtime_module.DiffusionPagedAttentionLayerAdapter(
        layer_name="layer-0",
        layer=layer,
        spec=spec,
        vllm_config=config,
    ...
    assert selected_backends == [(runtime_module.AttentionBackendEnum.FLASH_ATTN, {})]
...
def test_omni_attention_wraps_paged_kernel_with_sp_hooks() -> None:
    ...
    with set_forward_context(), override_paged_kv_runtime(Adapter()):
