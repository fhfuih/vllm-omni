...
def test_paged_scheduler_requires_a_worker_sequence_limit() -> None:
    with pytest.raises(ValueError, match="requires diffusion_kv_max_sequences_per_request"):
...
def test_worker_sequence_limit_must_be_a_positive_integer(invalid_limit: object) -> None:
    with pytest.raises((TypeError, ValueError)):
        OmniDiffusionConfig.from_kwargs(diffusion_kv_max_sequences_per_request=invalid_limit)
    with pytest.raises((TypeError, ValueError)):
        _DiffusionConfigProjection.from_kwargs(diffusion_kv_max_sequences_per_request=invalid_limit)
