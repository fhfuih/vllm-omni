...
@pytest.mark.core_model
@pytest.mark.cpu
def test_set_forward_context_only_manages_diffusion_state():
    ...
    with set_forward_context(omni_diffusion_config=od_config):
        ...
        assert get_forward_context().omni_diffusion_config is od_config
...
@pytest.mark.core_model
@pytest.mark.cpu
def test_initialize_vllm_ir_runtime_sets_worker_lifetime_defaults(mocker):
    from vllm_omni.diffusion.worker import diffusion_worker

    set_default = mocker.Mock()
    set_default_torch_wrap = mocker.patch.object(diffusion_worker.vllm.ir, "set_default_torch_wrap")
    vllm_config = SimpleNamespace(
        kernel_config=SimpleNamespace(ir_op_priority=SimpleNamespace(set_default=set_default)),
        compilation_config=SimpleNamespace(ir_enable_torch_wrap=True),
    )

    diffusion_worker._initialize_vllm_ir_runtime(vllm_config)

    set_default.assert_called_once_with()
    set_default_torch_wrap.assert_called_once_with(True)
