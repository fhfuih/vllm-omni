...
from .utils.validators import add_sampling_parameters_to_stage, validate_model_config, validate_sampling_params_types
...
class _VLLMOmniGenerateBase:
    ...
    def _validate_and_prepare_chat_completion_payload(
        self, model, prompt, negative_prompt, sampling_params, image=None, audio=None, video=None
    ):
        """Common validation and payload preparation logic."""
        ...
        **kwargs,
        ...
        if (
            ...
            if audio is None and image is None and video is None:
                ...
                return (output,)
            ### DALL-E image edit is not yet supported
            # elif image is not None and audio is None and video is None:
            #     print("DEBUG: Using DALL-E image edit endpoint")
            #     output = await client.edit_image(model, prompt, image, width, height, negative_prompt, mask, sampling_params)
            #     return (output,)
...
class VLLMOmniGenerateVideo(_VLLMOmniGenerateBase):
    ...
    def generate(self, url, model, prompt, negative_prompt, width, height, num_frames, fps, sampling_params, image=None, audio=None):
...
class VLLMOmniGenerateAudio(_VLLMOmniGenerateBase):
    ...
    def generate(self, url, model, prompt, negative_prompt, sampling_params, image=None, audio=None, video=None):
        raise NotImplementedError()
        client = self._get_client(url)

        payload = self._validate_and_prepare_chat_completion_payload(
            model, prompt, negative_prompt, sampling_params, image=image, audio=audio, video=video
        )
...
class VLLMOmniDiffusionSampling:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "n": ("INT", {"default": 1, "min": 0, "max": 10, "step": 1, "tooltip": "Number of images to generate"}),
                "num_inference_steps": (
                    "INT",
                    {"default": 50, "min": 1, "max": 1000, "tooltip": "Number of denoising steps (higher = better quality, slower)."},
                ),
                "guidance_scale": (
                    "FLOAT",
                    {
                ...
                "vae_use_slicing": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "Enable VAE slicing for reduced memory usage (slight quality trade-off)"},
                ),
            }
        }
