...
class DiffusersAdapterPipeline(nn.Module, DiffusionPipelineProfilerMixin):
    ...
    def load_weights(self) -> None:
        ...
        if self.od_config.enable_layerwise_offload:
            self._pipeline.enable_sequential_offloading()
    ...
    def _set_attention_backend(self) -> None:
        ...
        match self.od_config.attention_backend:
            case "FLASH_ATTN":
            ...
            case "TORCH_SDPA":
                attention_backend_attempts.append("native")
            case _:
                logger.warning(f"Invalid attention backend: {self.od_config.attention_backend}. Falling back to SDPA.")
                attention_backend_attempts.append("native")
        ...
        if len(attempt_errors) == len(attention_backend_attempts):
            ...
            logger.warning(
                f"Failed to set attention backend '{self.od_config.attention_backend}' for "
        ...
        logger.info(
            ...
            f"user config value '{self.od_config.attention_backend}'."
        )
