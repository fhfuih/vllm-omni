...
from vllm_omni.diffusion.diffusion_kv.v1.connector import start_load_kv, wait_remote_kv_before_forward
...
class DiffusionModelRunner(OmniConnectorModelRunnerMixin):
    ...
    def __init__(
        ...
        # live on the paged Worker backend as ``kv_caches_by_layer``.
        self.kv_cache_config: KVCacheConfig | None = None
        # Populated by DiffusionKVModelRunnerBackend (#6102). Empty until then.
        self.diffusion_kv_backend: Any | None = None
    ...
    def _uses_native_kv_load(self, scheduler_output: DiffusionSchedulerOutput | None) -> bool:
        return scheduler_output is not None and scheduler_output.kv_connector_metadata is not None

    def _maybe_load_remote_kv(self, scheduler_output: DiffusionSchedulerOutput):
        """Bind metadata, start_load_kv, and block until this rank's pages arrive."""
        metadata = scheduler_output.kv_connector_metadata
        if metadata is None or not scheduler_output.scheduled_new_reqs:
            return None
        new_request_ids = [new_req.request_id for new_req in scheduler_output.scheduled_new_reqs]
        start_load_kv(metadata)
        timeout = self.kv_transfer_manager.config.recv_timeout
        return wait_remote_kv_before_forward(new_request_ids, timeout=timeout)

    def _should_receive_omnikv(
        self,
        req: OmniDiffusionRequest | None = None,
        *,
        scheduler_output: DiffusionSchedulerOutput | None = None,
    ) -> bool:
        """Skip OmniKV packed receive when native connector load already admitted."""
        if self._uses_native_kv_load(scheduler_output):
            return False
        cache_mode = getattr(self.od_config, "diffusion_kv_mode", DiffusionKVCacheMode.DENSE_LEGACY)
        if cache_mode is DiffusionKVCacheMode.PAGED_SCHEDULER and self.od_config.kv_transfer_config is not None:
            return False
        if req is not None and req.kv_transfer_params:
            return False
        return True
    ...
    def kv_caches_by_layer(self) -> dict[str, torch.Tensor]:
        """Layer-name KV tensors owned by the paged Worker backend, if present."""
        backend = self.diffusion_kv_backend
        if backend is None:
            return {}
        caches = backend.kv_caches_by_layer
        return caches if caches else {}

    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        """Allocate Worker pages after Engine sizing. No-op until #6102 backend exists."""
        backend = self.diffusion_kv_backend
        if backend is None:
            return
        backend.initialize_kv_cache(kv_cache_config)
    ...
    def _prepare_request_for_forward(
        ...
        *,
        ...
        if self._should_receive_omnikv(req, scheduler_output=scheduler_output):
            ...
            logger.debug("Skipping OmniKV receive for native paged load request %s", req.request_id)

        # Kick off the next request's prefetch (+ H2D) to overlap this forward.
        if (
            self._should_receive_omnikv(req, scheduler_output=scheduler_output)
            and use_prefetch
            and self._kv_prefetch_enabled
            and kv_prefetch_job is not None
        ):
    ...
    def _execute_request_list(
        ...
        *,
        ...
        if scheduler_output is not None:
            kv_connector_output = self._maybe_load_remote_kv(scheduler_output)
        ...
        runner_output = self._runner_output_from_outputs(reqs, outputs)
        runner_output.kv_connector_output = kv_connector_output
    ...
    def _update_states(self, scheduler_output: DiffusionSchedulerOutput) -> tuple[list[StepRequestState], list[str]]:
        ...
        try:
            ...
            for sched_new_req in scheduler_output.scheduled_new_reqs:
                ...
                if self._should_receive_omnikv(state_req, scheduler_output=scheduler_output):
                    self.kv_transfer_manager.receive_multi_kv_cache_distributed(
                        state_req,
        ...
        *,
        ...
        with grad_context:
            ...
            with set_forward_context(
                ...
                return BatchRunnerOutput.from_list(runner_output_list, kv_connector_output=kv_connector_output)
